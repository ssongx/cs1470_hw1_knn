...
class KNN_Model(KNN_ConfMtx):
    ...
    def get_neighbor_counts(self, new_image, return_indices=False):
        ...
        difference = self.image_train.copy() - new_image
        ...
        lowest_distance = np.argpartition(distances, self.k_neighbors)        
        nearest_indices = lowest_distance[0:self.k_neighbors]

        nearest_label = []

        for i in range(0, self.k_neighbors):
            nearest_label.append(self.label_train[nearest_indices[i]])
            
        class_counts = []

        for i in range(0, len(self.class_list)):
            count = 0
            for j in range(0, self.k_neighbors):
                if nearest_label[j] == self.class_list[i]:
                    count = count + 1
    ...
    def predict(self, new_image):
        ...
        highest_count = np.argmax(self.get_neighbor_counts(new_image, return_indices=False))
        prediction_label = self.class_list[highest_count]
    ...
    def get_prediction_array(self, image_test):
        ...
        """
        prediction_list = []
        for i, each_image in enumerate(image_test.copy()):
            # TODO #1: Populate the prediction_list array.
            #each_image = np.reshape(each_image, (1, 784))
            ...
            if i % 10 == 0:
               print(f"check {i}'s iterations")
               print(self.predict(each_image))
            #continue

        return np.array(prediction_list)

    def get_prediction_acc(self, prediction_array, actual_class):
        """
        """
        count = 0
        for i in range(0, len(prediction_array) - 1):
            if prediction_array[i] == actual_class[i]:
                count = count + 1
        return count / len(prediction_array)
