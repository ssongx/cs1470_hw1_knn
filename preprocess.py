...
def get_data_MNIST(subset, data_path="../data"):
    ...
    #read the zipped image file
    image_file = open(inputs_file_path, 'rb')
    unzipped = gzip.GzipFile(fileobj = image_file)

    # normalize and flatten the image file
    unzipped.read(16)
    unzipped_file = np.frombuffer(unzipped.read(num_examples * 784), np.uint8)
    unzipped_file = np.float32(unzipped_file / 255)
    image = np.reshape(unzipped_file, (num_examples, 784))

    # type-convert the image

    ## TODO: read the label file
    label_file = open(labels_file_path, 'rb')
    unzipped_label = gzip.GzipFile(fileobj = label_file)
    unzipped_label.read(8)
    label = np.frombuffer(unzipped_label.read(num_examples), np.uint8)
...
def get_data_CIFAR(subset, data_path="../data"):
    ...
    cifar_meta = pickle.load(open(data_meta), encoding = 'bytes')

    for file in open(data_files):
        cifar_dict_batch = pickle.load(file, encoding = 'bytes')
        cifar_dict[b'data'].append(cifar_dict_batch[b"data"])
        cifar_dict[b'labels'].append(cifar_dict_batch[b"labels"])
    ...
    label = [l.decode('utf-8') for l in label_names]
    ...
    image = np.reshape(image, (len(image), 32, 32, 3))
...
def get_specific_class(image_full, label_full, specific_class=0, num=None):
    ...
    n = 0
    while i < num:
        if label_full[n] == specific_class:
            image[i] = image_full[n]
            label[i] = specific_class
            i = i + 1
        n = n + 1
...
def get_subset(image_full, label_full, class_list=list(range(10)), num=100):
    ...
    for i in range(len(class_list)):
        temp_image, temp_label = get_specific_class(image_full, label_full, specific_class = class_list[i], num = num)
    ...
    image = np.concatenate(image_list, dtype = np.float32)
    label = np.concatenate(np.concatenate(label_list))
